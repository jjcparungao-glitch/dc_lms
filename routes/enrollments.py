...
def get_enrollments():
    try:
        ...
        with db.cursor() as cursor:
            ...
            valid_sort_columns = ['external_id', 'full_name',
                                  'course_code', 'course_title',
                                  'term_code', 'created_at']
    ...
    try:
        ...
        with db.cursor() as cursor:
            ...
            if search:
                ...
                JOIN courses_master cm ON ci.course_id = cm.course_id
                WHERE cm.course_code LIKE %s
                OR cm.course_title LIKE %s
                OR ci.term_code LIKE %s
        ...
        return jsonify({
            'success': False,
            'message': 'An error occurred while fetching course instances',
            'error': str(e)
        }), 500
    ...
    try:
        ...
        with db.cursor() as cursor:
            ...
            if search:
                ...
                (external_id LIKE %s OR full_name LIKE %s)
...
def create_enrollment():
    try:
        ...
        if not instance_id or not user_id:
            return jsonify({
                'success': False,
                'message': 'instance_id and user_id are required',
...
def bulk_enroll_students():
    try:
        ...
        with db.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT instance_id FROM course_instances WHERE instance_id = %s",
                (instance_id,))
            ...
            for user_id in user_ids:
                try:
                    ...
                    cursor.execute(
                        "SELECT user_id, external_id FROM users WHERE user_id = %s",
                        (user_id,))
                    user = cursor.fetchone()
        ...
        return jsonify({
            'success': False,
            'message': f'An error occurred during bulk enrollment',
            'error': str(e)
        }), 500
...
def upload_csv():
    try:
        ...
        if not instance_id:
            return jsonify({'success': False, 'message': 'Course instance ID is required', 'error': 'Course instance ID is required'}), 400
        ...
        if not file.filename.lower().endswith('.csv'):
            return jsonify({'success': False, 'message': 'Please select a CSV file', 'error': 'Invalid file type'}), 400
        ...
        if file_size > 5 * 1024 * 1024:  # 5MB
            return jsonify({'success': False, 'message': 'File too large (max 5MB)', 'error': 'File too large (max 5MB)'}), 400
        ...
        with db.cursor() as cursor:
            ...
            if not instance:
                return jsonify({'success': False, 'message': 'Course instance not found', 'error': 'Course instance not found'}), 404
        ...
        return jsonify({
            'success': False,
            'message': f'Error processing CSV file',
            'error': str(e)
        }), 500
...
def export_csv():
    try:
        ...
        with db.cursor() as cursor:
            ...
            # Write header
            writer.writerow(['USN', 'Student Name',
                             'Course Code',
                             'Course Title', 'Term'])
