    ...
    try: 
        ...
        with db.cursor() as cursor:
            ...
            if is_select:
                ...
                return jsonify({'success': True, 'columns': columns, 'results': results, 'row_count': len(results)}), 200
            else:
                db.commit()
                return jsonify({'success': True, 'message': 'Query executed successfully', 'affected_rows': cursor.rowcount}), 200
    except Exception as e:
