...
from flask_jwt_extended import create_access_token, decode_token, get_jwt, jwt_required, get_jwt_identity
import bcrypt
        ...
        try:
            with db.cursor() as cur:
                ...
                if user and bcrypt.checkpw(password.encode('utf-8'), user['password_hash'].encode('utf-8')):
                    ...
                    # Check current active token count
                    cur.execute(
                        '''SELECT COUNT(*) as token_count FROM access_tokens
                           WHERE user_id = %s AND expires_at > %s''',
                        (user['user_id'], datetime.now(timezone.utc))
                    )
                    token_count = cur.fetchone()['token_count']

                    # If at limit, remove oldest token
...
@jwt_required()
def logout():
    jti = get_jwt()['jti']  # Get JTI from current token
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute("DELETE FROM access_tokens WHERE jti = %s", (jti,))
            db.commit()
